...
void Print_To_LCD_DELAY(const char *);
...
void Print_To_LCD_DELAY(const char * tempPtr)
{
	char currentChar;
    PORTEbits.RE0 = 0;          // Drive RS pin low for cursor-positioning code
...
//prints concurrent chars to screen, alternating lines between output. Clear each line before outputting.
void Part1(void){
    char curr = '!';
    char currentChar;
    PORTEbits.RE0 = 0;          // Drive RS pin low for cursor-positioning code
    ...
    while(1) {
		...
		//print to line one
        Print_To_LCD(Clear1);
        Print_To_LCD_DELAY(Str_1);
        
		//print to line two
        Print_To_LCD(Clear2);
        Print_To_LCD_DELAY(Str_2);

    }
}

//prints given hex values of chars for equations
void Part2(void){
    char Str_3[] = {0x80,'X','=',0xE0,'*',0xF6,0xE2,'*','Y',0};
    Print_To_LCD(Str_3);
    __delay_ms(3000);
    char Str_4[] = {0x80,'Y','=','s','i','n','(',0xF4,')',0};
    Print_To_LCD(Str_4);
        __delay_ms(3000);

}

//creates user-defined char '~'
void Part3(void){
	//hex values of each line of LED display
    char var[] = {0x58, 0x80, 0x80, 0x8C, 0x95, 0x86, 0x80, 0x80, 0};
	
	//send char to memory location 0x58
    Print_To_LCD(var);
...
//creates user defined char ' '. same process as Part3()
void Part4(void){
    char var[] = {0x58, 0x80, 0x84, 0x9F, 0x95, 0x9F, 0x84, 0x84, 0};
    Print_To_LCD(var);
    Str_1[4] = 0x03;
    Print_To_LCD(Str_1);
}
